...
import { AlertContext, Dialogue, DialogueContext, ModalContext, useBgm, useGameData, useGameStage, useGameTime, usePage, useStats, useTime } from "../../util/hooks";
...
import { DialogueHandler } from "../../components/DialogueHandler";
...
export const GamePage = () => {
	...
	const [dialogue, setDialogue] = useState<{ id: number; dialogue: Dialogue }[]>([]);
	const modalContext = useMemo(() => ({ modals, setModals }), [modals, setModals]);
	const alertContext = useMemo(() => ({ alerts, setAlerts }), [alerts, setAlerts]);
	const dialogueContext = useMemo(() => ({ dialogue, setDialogue }), [dialogue, setDialogue]);
		...
		<main className={styles.main}>
			<AlertContext.Provider value={alertContext}>
				<ModalContext.Provider value={modalContext}>
					<DialogueContext.Provider value={dialogueContext}>
						<div className={styles.gameBox}>
							...
							<div className={styles.gameTime}>
								{gameTime.month[0].toUpperCase() + gameTime.month.slice(1)} {gameTime.date} (Day {gameTime.day})
								{gameTime.block !== "-" ? ` - Block ${gameTime.block}` : ""}
							</div>
